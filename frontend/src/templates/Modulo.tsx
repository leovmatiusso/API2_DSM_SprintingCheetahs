    ...
    <main className={`page ${pageClassName}`.trim()}>
      ...
      <section className="card content">
        <div className="module-heading">
          <div>
            <p className="eyebrow">Navegação / protótipo</p>
            <h1>{title}</h1>
          </div>
          <span className="bg-secondary rounded-full px-3 py-2 text-xs whitespace-nowrap">
            Somente navegação
          </span>
        </div>
        ...
        {items.length > 0 && (
          <div className="future-list">
            <h2>O que será desenvolvido nesta tela</h2>
            <ul>
              {items.map((item) => (
                <li key={item}>{item}</li>
              ))}
        ...
        <div className="info">
          Esta versão serve como base visual e de navegação. As operações reais serão implementadas
          nas próximas etapas.
        </div>
      </section>
    </main>
