...
app.get("/api/health", (_req, res) => res.json({ message: "Backend funcionando!" }));
...
app.get("/api/account", requireAuth, (_req, res) => {
  res.json({ user: publicUser(res.locals.user as User) });
});
...
app.put("/api/change-password", requireAuth, (req, res) => {
  ...
  if (newPassword.length < 6) return res.status(400).json({ message: "A nova senha deve possuir pelo menos 6 caracteres." });

  const user = res.locals.user as User;
  if (user.password !== currentPassword) return res.status(401).json({ message: "A senha atual está incorreta." });
  user.password = newPassword;
  return res.json({ message: "Senha atualizada com sucesso." });
});

// ADMINISTRAÇÃO DE CONTAS — exclusiva do Superusuário.
app.get("/api/users", requireRole("superusuario"), (_req, res) => {
  res.json({ users: users.map(publicUser) });
});

app.post("/api/users", requireRole("superusuario"), (req, res) => {
  const { name, email, password, role } = req.body as { name?: string; email?: string; password?: string; role?: Role };
  const validRoles: Role[] = ["superusuario", "gestor", "comercial", "suporte", "producao", "software", "implantacao"];
  if (!name || !email || !password || !role) return res.status(400).json({ message: "Nome, email, senha e perfil são obrigatórios." });
  ...
  if (users.some(user => user.email.toLowerCase() === email.trim().toLowerCase())) return res.status(409).json({ message: "Este email já está cadastrado." });

  const user: User = { id: crypto.randomUUID(), name: name.trim(), email: email.trim().toLowerCase(), password, role, active: true };
  users.push(user);
  res.status(201).json({ user: publicUser(user) });
});

app.put("/api/users/:id", requireRole("superusuario"), (req, res) => {
  const user = users.find(item => item.id === req.params.id);
  if (!user) return res.status(404).json({ message: "Usuário não encontrado." });

  const { name, email, password, role, active } = req.body as Partial<User>;
  const validRoles: Role[] = ["superusuario", "gestor", "comercial", "suporte", "producao", "software", "implantacao"];
  if (role !== undefined && !validRoles.includes(role)) return res.status(400).json({ message: "Perfil inválido." });
  if (password !== undefined && password.length < 6) return res.status(400).json({ message: "A senha deve possuir pelo menos 6 caracteres." });
  if (email !== undefined && users.some(item => item.id !== user.id && item.email.toLowerCase() === email.trim().toLowerCase())) return res.status(409).json({ message: "Este email já está cadastrado." });
  ...
  // Invalida sessões existentes quando a conta é desativada.
  if (!user.active) {
...
app.delete("/api/users/:id", requireRole("superusuario"), (req, res) => {
  const currentUser = res.locals.user as User;
  if (currentUser.id === req.params.id) return res.status(400).json({ message: "O próprio usuário logado não pode ser excluído." });
